...
module softmax_core_hls_ip (
  ...
  assign out_chunk_o = tb_softmax_wrapper.core_prob_chunks_mem[out_idx_q];
  ...
  always_ff @(posedge ap_clk) begin
    ...
    if (!ap_rst_n) begin
      ...
      unique case (state_q)
        CORE_IDLE: begin
          if (start_i) begin
            if (key_col_count_i != COUNT_W'(SCORE_K_TILE)) begin
              $error("softmax_core_hls_ip expected wrapper to pass full SCORE_K_TILE");
              $finish;
            end
            in_idx_q <= '0;
            out_idx_q <= '0;
        ...
        CORE_INPUT: begin
          if (in_valid_i && in_ready_o) begin
            if (in_chunk_i !== tb_softmax_wrapper.core_score_chunks_mem[in_idx_q]) begin
              $error("softmax_core_hls_ip input chunk mismatch at %0d", in_idx_q);
              $display("expected=%h", tb_softmax_wrapper.core_score_chunks_mem[in_idx_q]);
              $display("actual  =%h", in_chunk_i);
              $finish;
            end
            if (in_idx_q == (tb_softmax_wrapper.CHUNK_COUNT - 1)) begin
              state_q <= CORE_OUT;
            end
        ...
        CORE_OUT: begin
          if (out_valid_o && out_ready_i) begin
            if (out_idx_q == (tb_softmax_wrapper.CHUNK_COUNT - 1)) begin
              done_pulse_o <= 1'b1;
              state_q <= CORE_IDLE;
            end
