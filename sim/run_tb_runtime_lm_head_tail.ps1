...
function Stage-FixturesForXsim {
  ...
  $fixtureStageDir = Join-Path $RunDirectory "fixtures"
  ...
  return "fixtures\$fixtureLeaf"
}

$scriptDir = Split-Path -Parent $MyInvocation.MyCommand.Path
$repoRoot = Split-Path -Parent $scriptDir
$stamp = Get-Date -Format "yyyyMMdd_HHmmss"
$logRootRelative = "sim\\logs\\tb_runtime_lm_head_tail\\$stamp"
$logRoot = Join-Path $repoRoot $logRootRelative
$resolvedFileListRelative = "$logRootRelative\\lmh_filelist_resolved.txt"
$resolvedFileList = Join-Path $repoRoot $resolvedFileListRelative
$sourceFileList = Join-Path $scriptDir "lmh_filelist.txt"
...
$null = Stage-FixturesForXsim -FixtureBaseResolved $fixtureBaseResolved -RunDirectory $logRoot

if ($Simulator -eq "iverilog") {
  $compileLog = Join-Path $logRoot "compile.log"
  $runLog = Join-Path $logRoot "run.log"
  $vvpPathRelative = "$logRootRelative\\tb_runtime_lm_head_tail.vvp"
  ...
  Write-Host "Compiling tb_runtime_lm_head_tail with Icarus into $vvpPathRelative"
  $compileExit = Invoke-NativeLogged `
    -FilePath "iverilog" `
    -ArgumentList @("-g2012", "-f", $resolvedFileListRelative, "-o", $vvpPathRelative) `
    -LogPath $compileLog `
  ...
  Write-Host "Running tb_runtime_lm_head_tail with Icarus from repo root"
  ...
  if (-not (Select-String -Path $runLog -Pattern "PASS: tb_runtime_lm_head_tail" -Quiet)) {
...
$libraryName = "lmh_$stamp"
$snapshotName = "tb_runtime_lm_head_tail_$stamp"
...
Write-Host "Compiling tb_runtime_lm_head_tail with XSIM in $logRootRelative"
$xvlogExit = Invoke-NativeLogged `
  -FilePath $xvlogBat `
  -ArgumentList (@("--nolog", "--sv", "-work", $libraryName) + $resolvedFiles) `
  -LogPath $xvlogLog `
...
Write-Host "Elaborating tb_runtime_lm_head_tail snapshot $snapshotName"
$xelabExit = Invoke-NativeLogged `
  -FilePath $xelabBat `
  -ArgumentList @("--nolog", "--timescale", "1ns/1ps", "$libraryName.tb_runtime_lm_head_tail", "-s", $snapshotName) `
  -LogPath $xelabLog `
  -WorkingDirectory $logRoot
...
Write-Host "Running tb_runtime_lm_head_tail with XSIM in $logRootRelative"
...
if (-not (Select-String -Path $xsimLog -Pattern "PASS: tb_runtime_lm_head_tail" -Quiet)) {
